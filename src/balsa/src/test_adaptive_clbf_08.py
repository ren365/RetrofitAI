...
# initial intermedian paras
# [mu_rm,mu_pd,mu_ad,mu_qp,mu_new]
mu_rm = np.zeros((udim,N))
mu_pd = np.zeros((udim,N))
mu_ad = np.zeros((udim,N))
mu_qp = np.zeros((udim,N))
mu_new = np.zeros((udim,N))
# end initial
...
for i in range(N-2):
    ...
    # # update barrier
	# tmpindx = min(6,int(i/(N*2/3)*4)+2) # this is 2. for 3: int(i/(N*1/2)*4)+3
	# barrier_x_tmp = barrier_x[:tmpindx]
	# barrier_y_tmp = barrier_y[:tmpindx]
	# adaptive_clbf.update_barrier_locations(barrier_x_tmp,barrier_y_tmp,params["barrier_radius"])
	...
	u[:,i+1],mu_rm[:,i+1],mu_pd[:,i+1],mu_ad[:,i+1],mu_qp[:,i+1] = \
        adaptive_clbf.get_control_safe_debug(z[:,i:i+1],z_d[:,i+1:i+2],z_d_dot,dt=dt,obs=np.concatenate([x_ad[2,i:i+1],u_ad[:,i]]),use_model=True,add_data=add_data,use_qp=True)
...
plt.rcParams.update({'font.size': 12})
plt.subplot(211)
plt.plot(t,mu_rm[0,:-1],'m--',alpha=0.9)
plt.plot(t,mu_pd[0,:-1],'b-',alpha=0.9)
plt.plot(t,mu_ad[0,:-1],'y--',alpha=0.9)
plt.plot(t,mu_qp[0,:-1],'r-,',alpha=0.9)
plt.plot(t,u[0,:-1],'g-',alpha=0.9)
plt.legend(['rm','pd','ad','qp','final'],bbox_to_anchor=(0,1.1,1,0.2), loc="upper center", ncol=4)
plt.plot([t[0],t[-1]],[params["steering_limit"],params["steering_limit"]],'k--')
plt.plot([t[0],t[-1]],[-params["steering_limit"],-params["steering_limit"]],'k--')
plt.ylabel('Steering Angle (rad)')
plt.subplot(212)
plt.plot(t,mu_rm[1,:-1],'m--',alpha=0.9)
plt.plot(t,mu_pd[1,:-1],'b-',alpha=0.9)
plt.plot(t,mu_ad[1,:-1],'y:',alpha=0.9)
plt.plot(t,mu_qp[1,:-1],'r-,',alpha=0.9)
plt.plot(t,u[1,:-1],'g-',alpha=0.9)
plt.plot([t[0],t[-1]],[params["min_accel"],params["min_accel"]],'k--')
plt.plot([t[0],t[-1]],[params["max_accel"],params["max_accel"]],'k--')
