...
for i in range(N-2):
	...
	if i < 100:
		u_balsa[:,i+1] = adaptive_clbf_balsa.get_control(z_balsa[:,i:i+1],z_d[:,i+1:i+2],z_d_dot,dt=dt,obs=np.concatenate([]),use_model=False,add_data=False,use_qp=True)
	else:
	...
	if i < 100:
		u_safety[:,i+1] = adaptive_clbf_safety.get_control(u_safety[:,i:i+1],z_d[:,i+1:i+2],z_d_dot,dt=dt,obs=np.concatenate([]),use_model=False,add_data=False,use_qp=True)
	else:
