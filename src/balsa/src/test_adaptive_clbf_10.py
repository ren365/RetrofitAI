...
for i in range(N-2):
	...
	u_safety[:,i+1] = adaptive_clbf_safety.get_control_safe(z_safety[:,i:i+1],z_d[:,i+1:i+2],z_d_dot,dt=dt,obs=np.concatenate([x_ad[2,i:i+1],u_ad[:,i]]),use_model=True,add_data=add_data,use_qp=True)
	if (i - start_training -1 ) % train_interval == 0 and i > start_training:
...
plt.semilogy(t[:-1],prediction_var_balsa[0,:-2],'g-',alpha=0.9)
plt.semilogy(t[:-1],prediction_var_ad[1,:-2],'m:',alpha=0.9)
plt.semilogy(t[:-1],prediction_var_balsa[1,:-2],'g:',alpha=0.9)
plt.xlabel("Time(s)")
...
plt.plot(t[:-1],prediction_error_true_balsa[:-2],'g:',alpha=0.9)
# plt.ylim([0,1.0])
...
# plt.plot(x_safety[0,:],x_safety[1,:],'c-',alpha=0.9,label='safety')
# plt.plot(x_balsa[0,:],x_balsa[1,:],'g-',alpha=0.9,label='balsa',linewidth=3.0)
...
plt.plot(t,u_safety[0,:-1],'c-',alpha=0.9)
...
plt.plot(t,u_safety[1,:-1],'c-',alpha=0.9)
...
plt.plot(t,np.sqrt((x_d[0,:]-x_safety[0,:])**2 + (x_d[1,:]-x_safety[1,:])**2),'c-',alpha=0.9)
...
plt.plot(t,x_safety[3,:],'c-',alpha=0.9)
...
plt.plot(t,barrier_dist_safety-params["barrier_radius"],'c-',alpha=0.9)
