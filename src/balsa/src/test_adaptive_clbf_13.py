...
for i in range(N-2):
	...
	if i < 85:
		u_balsa[:,i+1] = adaptive_clbf_balsa.get_control(z_balsa[:,i:i+1],z_d[:,i+1:i+2],z_d_dot,dt=dt,obs=[],use_model=False,add_data=add_data,use_qp=True)
	else:
	...
	if i < 85:
		u_safety[:,i+1] = adaptive_clbf_safety.get_control(z_safety[:,i:i+1],z_d[:,i+1:i+2],z_d_dot,dt=dt,obs=[],use_model=False,add_data=add_data,use_qp=True)
	else:
