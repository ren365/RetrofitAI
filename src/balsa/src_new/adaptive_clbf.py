...
class AdaptiveClbf(object):
	...
	def get_control_safe_debug(self,z,z_ref,z_ref_dot,dt,obs=None,train=False,use_model=False,add_data=True,check_model=True,use_qp=True):
		...
		# add debug return
		mu_qp = self.mu_qp
		mu_rm = [np.arctan(mu_rm[0] * self.vehicle_length),mu_rm[1]]
		mu_pd = [np.arctan(mu_pd[0] * self.vehicle_length),mu_pd[1]]
		mu_ad = [np.arctan(mu_ad[0] * self.vehicle_length),mu_ad[1]]
		mu_qp = [np.arctan(mu_qp[0] * self.vehicle_length),mu_qp[1]]

		# return self.controls
		return self.controls,mu_rm,mu_pd,mu_ad,mu_qp